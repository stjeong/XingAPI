...
robocopy %CURRENTDIR%..\Lib %CURRENTDIR%nuget\root\build\x86 /S /XD AutoGenerated
robocopy %CURRENTDIR%nuget_support %CURRENTDIR%nuget\root\build XingAPINet.targets
robocopy %CURRENTDIR% %CURRENTDIR%nuget\root %PRJNAME%.nuspec

msbuild %CURRENTDIR%..\XingAPINet.Sqlite\XingAPINet.Sqlite.csproj /p:Configuration=%BUILDCONFIG%;DefineConstants="TRACE" /t:Rebuild
if ERRORLEVEL 1 goto BuildError
robocopy %CURRENTDIR%..\XingAPINet.Sqlite\bin\%BUILDCONFIG%\netstandard2.0 %CURRENTDIR%nuget\root\lib\net40 XingAPINet.Sqlite.* /XF *.deps.json
